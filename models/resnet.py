...
class MTResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layer3_ = copy.deepcopy(self.layer3)
        self.layer4_ = copy.deepcopy(self.layer4)
        self.avgpool_ = nn.AdaptiveAvgPool2d((1, 1))
        # self.fc_ = nn.Linear(512 * block.expansion, num_classes)
        self.fc_ = copy.deepcopy(self.fc)
...
def _mt_resnet(
    ...
    **kwargs: Any
    ...
    model = MTResNet(block, layers, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict)
...
def mt_resnet18(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> MTResNet:
    ...
    """
    return _mt_resnet('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)
